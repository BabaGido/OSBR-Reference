    ...
    try:
        ...
        if not filtered_data.empty:
            ...
            if selected_calculation == "Average COGM Cost/Lot":
                ...
                total_cogm = filtered_data["cogmcost"].sum()  # Total COGM in $
                total_units = filtered_data["unitsmanufactured"].sum()  # Total Units
                cost_per_sold_unit = (total_cogm / total_units) if total_units != 0 else 0

                # Display results
                st.write("### Calculation Results")
                st.write(f"**Total COGM for Selected Product ({selected_product_name}):** ${total_cogm:,.2f}")
                st.write(f"**Total Units for Selected Product ({selected_product_name}):** {total_units:,}")
                st.write(f"**Cost per Sold Unit for Selected Product ({selected_product_name}):** ${cost_per_sold_unit:,.2f}")
        else:
